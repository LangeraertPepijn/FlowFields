...
void App_AgarioGame_BT::Start()
{
	...
	for (int i = 0; i < m_AmountOfAgents; i++)
	{
		...
		BehaviorTree* pBT = new BehaviorTree(pB, new BehaviorAction(ChangeToWander));

		newAgent->SetDecisionMaking(pBT);
	...
	//add the decision making for uber agent
	...
	m_pUberAgent->SetDecisionMaking(pBT);
}
