	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		if (m_PropagationInterval < m_TimeSinceLastPropagation)
		{
			for (size_t i{};i<m_Nodes.size();i++)
			{
				...
				for (auto& connection : GetNodeConnections(m_Nodes[i]->GetIndex()))
				{
				
					float temp = m_Nodes[connection->GetTo()]->GetInfluence() * expf(-connection->GetCost() * m_Decay);
					if (abs(newInfluence) < abs(temp))
						if (m_MaxAbsInfluence < abs(temp))
							newInfluence = m_MaxAbsInfluence;
						else
							newInfluence = temp;
				

				}
				float temp = m_Nodes[i]->GetInfluence() * expf(-1 * m_Decay);
				if (abs(newInfluence) < abs(temp))
					if (abs(m_MaxAbsInfluence) < abs(temp))
						newInfluence = m_MaxAbsInfluence;
					else
						newInfluence = temp;
				
				m_InfluenceDoubleBuffer[i] =Elite::Lerp( m_InfluenceDoubleBuffer[i], newInfluence, m_Momentum);
