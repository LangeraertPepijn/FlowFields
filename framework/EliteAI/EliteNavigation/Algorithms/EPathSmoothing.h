	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				if (0 >= Cross(newRightLeg, rightLeg))
				{
					//2. See if new line degenerates a line segment - RIGHT
					if (0 <= Cross(newRightLeg, leftLeg))
					{
					...
					else
					{
						...
						unsigned int newIt{ (unsigned int)(apexIndex + 1) };
						leftLegIndex = newIt;
						rightLegIndex = newIt;
						i = newIt;
				...
				if (0 <= Cross(newLeftLeg, leftLeg))
				{
					//2. See if new line degenerates a line segment - LEFT
					if (0 >= Cross(newLeftLeg, rightLeg))
					{
						leftLeg = newLeftLeg;
						leftLegIndex = i;
					}
			...
			auto lastPortal = *(portals.end()-1);
			vPath.push_back(lastPortal.Line.p1);
			return vPath;
		}
	private:
		SSFA() {};
